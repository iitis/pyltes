...
class UE(NetworkDevice):
    ...
    def calculateReceivedPower(self, pSend, distance):
        ...
        lambda_val = 0.42857142857142855
        a = 4.0
        b = 0.0065
        c = 17.1
        d = 10.8
    ...
    def calculateSINRfor(self, where, BS_vector, obstacleVector = None, useRayleigh = False):
    ...
    def calculateSINR(self, BS_vector, obstacleVector = None, useRayleigh = False):

        SINRin = self.calculateSINRfor("in", BS_vector, obstacleVector, useRayleigh)
        if(SINRin > BS_vector[self.connectedToBS].mi):
            SINR=SINRin
            self.inside = True
        else:
            SINR=self.calculateSINRfor("out", BS_vector, obstacleVector, useRayleigh)
            self.inside = False
    ...
    def calculateMaxThroughputOfTheNode(self, bs_vector, obstacles = None):
        r_i = 0.0
        M_i = 0.0
        sinr = self.calculateSINR_brno(bs_vector, obstacles)
        if sinr < -5.45:
            r_i = 0
            M_i = 1
        elif -5.45 <= sinr < -3.63:
            ...
            #capacityForUE_s = capacityForUE_ms * 1000
        capacityForUE_ms = r_i * math.log2(M_i) * 12 * 7 * (3/2*(50/1))
        capacityForUE_s = capacityForUE_ms * 1000
